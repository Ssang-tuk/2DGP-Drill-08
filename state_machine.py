...
class StateMachine:
    ...
    # state_event 가 어떤 이벤트인지 체크할 수 있어야함.
        for check_event in self.rules[self.cur_state].keys():
            if check_event(state_event):
                ...
                print( f'State Change: {self.cur_state.__class__.__name__} -> {self.next_state.__class__.__name__} by {event_to_string(state_event)}' )
